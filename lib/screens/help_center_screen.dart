...
class _HelpCenterScreenState extends State<HelpCenterScreen> with SingleTickerProviderStateMixin {
      ...
      'questions': [
        {
          ...
          'a': 'Pilih produk lelang yang Anda inginkan, masukkan jumlah penawaran Anda di atas harga saat ini, lalu tekan tombol "Tawar". Pastikan saldo e-wallet Anda mencukupi atau bersiap melakukan pembayaran ketika waktu lelang berakhir.'
        },
        {
          'q': 'Apakah penawaran yang sudah diajukan bisa dibatalkan?',
          'a': 'Demi menjaga keadilan dan komitmen, penawaran lelang yang sudah diajukan tidak dapat dibatalkan. Mohon lakukan bid dengan bijak.'
        },
        {
          'q': 'Bagaimana jika saya memenangkan lelang?',
          'a': 'Anda akan menerima notifikasi kemenangan lelang. Anda memiliki waktu 24 jam untuk menyelesaikan pembayaran produk lelang tersebut melalui menu pembayaran.'
        }
      ]
    },
    {
      ...
      'questions': [
        {
          ...
          'a': 'Kami mendukung berbagai metode pembayaran aman termasuk e-wallet (GoPay, DANA, ShopeePay), Virtual Account Bank (BCA, Mandiri, BNI), serta kartu debit dan kredit.'
        },
        {
          'q': 'Apakah transaksi di Thriftin aman?',
          'a': 'Tentu saja! Kami menggunakan sistem Rekening Bersama (Escrow). Dana Anda akan kami tahan dengan aman dan hanya dilepaskan ke penjual setelah Anda mengonfirmasi barang diterima dengan baik.'
        }
      ]
    },
    {
      ...
      'questions': [
        {
          ...
          'a': 'Estimasi pengiriman tergantung pada jenis kurir yang Anda pilih saat checkout dan lokasi penjual. Biasanya memakan waktu 2-5 hari kerja.'
        },
        {
          'q': 'Bagaimana cara melacak pesanan saya?',
          'a': 'Masuk ke halaman "Pesanan Saya", pilih detail pesanan yang ingin Anda lacak, lalu klik tombol "Lacak" untuk melihat status pengiriman kurir secara real-time.'
        }
      ]
    }
  ];
  ...
  Future<void> _loadInquiryHistory() async {
    ...
    try {
      ...
      final results = await db.query('help_messages', orderBy: 'id DESC');
      setState(() {
  ...
  Future<void> _submitInquiry() async {
    ...
    try {
      final db = await _dbHelper.database;
      final id = await db.insert('help_messages', {
        'name': name,
        'email': email,
          ...
          child: filteredFAQ.isEmpty
              ? Center(
                  child: Column(
                    ...
                    children: [
                      Icon(Icons.search_off, size: 48, color: AppColors.grey300),
                      const SizedBox(height: 12),
                      Text(
                        'FAQ tidak ditemukan',
  ...
  Widget _buildContactTab() {
    final categories = ['Masalah Lelang', 'Masalah Pembayaran', 'Akun & Keamanan', 'Lainnya'];
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Icon(Icons.headset_mic_outlined, size: 48, color: Colors.white.withValues(alpha: 0.9)),
              ],
            ),
          ),
          const SizedBox(height: 24),
          ...
          Form(
            ...
            child: Column(
              ...
              children: [
                // Name
                Text('Nama Lengkap', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textSecondary)),
                const SizedBox(height: 8),
                TextFormField(
                ...
                // Email
                Text('Alamat Email', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textSecondary)),
                const SizedBox(height: 8),
                TextFormField(
                ...
                // Category dropdown
                Text('Kategori Masalah', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textSecondary)),
                const SizedBox(height: 8),
                DropdownButtonFormField<String>(
                  ...
                  items: categories.map((cat) {
                    return DropdownMenuItem<String>(
                      value: cat,
                      child: Text(cat, style: TextStyle(fontSize: 14, color: AppColors.textPrimary)),
                    );
                  }).toList(),
                  onChanged: (val) {
                    if (val != null) {
                ...
                // Description message
                Text('Deskripsi Masalah', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textSecondary)),
                const SizedBox(height: 8),
                TextFormField(
                  controller: _messageController,
                  maxLines: 4,
                  decoration: InputDecoration(
                    hintText: 'Jelaskan secara rinci kendala yang Anda alami...',
                    filled: true,
                    fillColor: AppColors.background,
                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: AppColors.border)),
                    enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: AppColors.border)),
                  ...
                  validator: (v) => (v == null || v.trim().length < 10) ? 'Harap jelaskan masalah minimal 10 karakter' : null,
                ),
                const SizedBox(height: 24),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: const Text('Kirim Laporan', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 14)),
                  ),
                ),
              ],
            ),
          ...
          const SizedBox(height: 12),
          _isLoadingHistory
              ? Center(child: Padding(padding: const EdgeInsets.all(8.0), child: CircularProgressIndicator(color: AppColors.primary)))
                            ...
                            children: [
                              Row(
                                ...
                                children: [
                                  ...
                                  Text(
                                    '#TKT-${1000 + id}',
                                    style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w600, color: AppColors.textHint),
                                  ),
                                ],
                              ),
